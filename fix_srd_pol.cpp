...
// /* The above code appears to be a comment in C++ programming language. It starts with /* and
// ends with */, indicating that it is a multi-line comment. The content of the comment is
// "FixSRDPol". It is not clear what the purpose or intention of this comment is without further
// context. */
FixSRDPol::FixSRDPol(LAMMPS *lmp, int narg, char **arg) :
...
void FixSRDPol::init()
{
  ...
  for (int m = 0; m < modify->nfix; m++) {
    if (strcmp(modify->fix[m]->style, "wall/srd") == 0) {
      ...
      wallexist = 1;
      wallfix = dynamic_cast<FixWallSRDPol *>(modify->fix[m]);
      nwall = wallfix->nwall;
...
void FixSRDPol::setup(int /*vflag*/)
{
  setup_bounds();
...
void FixSRDPol::post_force(int /*vflag*/)
{
  int i, m, ix, iy, iz;
...
void FixSRDPol::reset_velocities()
{
...
void FixSRDPol::vbin_comm(int ishift)
{
...
void FixSRDPol::vbin_pack(BinAve *vbin, int n, int *list, double *buf)
{
  int j;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRDPol::vbin_unpack(double *buf, BinAve *vbin, int n, int *list)
{
  int j;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRDPol::xbin_comm(int ishift, int nval)
{
  BinComm *bcomm1, *bcomm2;
  MPI_Request request1, request2;
...
void FixSRDPol::xbin_pack(BinAve *vbin, int n, int *list, double *buf, int nval)
{
  int j, k;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRDPol::xbin_unpack(double *buf, BinAve *vbin, int n, int *list, int nval)
{
  int j, k;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRDPol::collisions_single()
{
...
void FixSRDPol::collisions_multi()
{
...
int FixSRDPol::inside_sphere(double *xs, double *xb, Big *big)
{
  double dx, dy, dz;
...
int FixSRDPol::inside_ellipsoid(double *xs, double *xb, Big *big)
{
  double x, y, z;
...
int FixSRDPol::inside_line(double *xs, double *xb, double *vs, double *vb, Big *big, double dt_step)
{
...
int FixSRDPol::inside_tri(double *xs, double *xb, double *vs, double *vb, Big *big, double dt_step)
{
...
int FixSRDPol::inside_wall(double *xs, int iwall)
{
...
double FixSRDPol::collision_sphere_exact(double *xs, double *xb, double *vs, double *vb, Big *big,
...
void FixSRDPol::collision_sphere_inexact(double *xs, double *xb, Big *big, double *xscoll,
...
double FixSRDPol::collision_ellipsoid_exact(double *xs, double *xb, double *vs, double *vb, Big *big,
                                         double *xscoll, double *xbcoll, double *norm)
{
...
void FixSRDPol::collision_ellipsoid_inexact(double *xs, double *xb, Big *big, double *xscoll,
                                         double *xbcoll, double *norm)
{
...
double FixSRDPol::collision_line_exact(double * /*xs*/, double * /*xb*/, double * /*vs*/,
...
double FixSRDPol::collision_tri_exact(double * /*xs*/, double * /*xb*/, double * /*vs*/,
                                   double * /*vb*/, Big * /*big*/, double dt_step, double *xscoll,
                                   double *xbcoll, double *norm)
{
  xscoll[0] = xsc[0];
...
double FixSRDPol::collision_wall_exact(double *xs, int iwall, double *vs, double *xscoll,
                                    double *xbcoll, double *norm)
{
  int dim = wallwhich[iwall] / 2;
...
void FixSRDPol::collision_wall_inexact(double *xs, int iwall, double *xscoll, double *xbcoll,
                                    double *norm)
{
  int dim = wallwhich[iwall] / 2;
...
void FixSRDPol::slip(double *vs, double *vb, double *xb, Big *big, double *xsurf, double *norm,
                  double *vsnew)
{
...
void FixSRDPol::slip_wall(double *vs, int iwall, double *norm, double *vsnew)
{
...
void FixSRDPol::noslip(double *vs, double *vb, double *xb, Big *big, int iwall, double *xsurf,
...
void FixSRDPol::force_torque(double *vsold, double *vsnew, double *xs, double *xb, double *fb,
                          double *tb)
{
...
void FixSRDPol::force_wall(double *vsold, double *vsnew, int iwall)
...
int FixSRDPol::update_srd(int i, double dt, double *xscoll, double *vsnew, double *xs, double *vs)
{
  int ix, iy, iz;
...
void FixSRDPol::parameterize()
{
  ...
  if (me == 0) {
   ...
   //mesg += fmt::print("   Epsilon = {:.8}\n", force->mvv2e);
    //mesg += fmt::print("   Kb = {:.8}\n", force->boltz);
    //mesg += fmt::print("   Kb = {:.8}\n", force->boltz);
    //mesg += fmt::print("   SRD mass = {:.8}\n", mass_srd);
    //mesg += fmt::print("   SRD timestep = {:.8}\n", dt_srd);  
...
void FixSRDPol::setup_velocity_bins()
{
...
void FixSRDPol::setup_velocity_shift(int ishift, int dynamic)
{
...
void FixSRDPol::setup_search_bins()
{
...
void FixSRDPol::setup_search_stencil()
{
...
double FixSRDPol::point_bin_distance(double *x, int i, int j, int k)
{
...
double FixSRDPol::bin_bin_distance(int i, int j, int k)
{
  double delx, dely, delz;
...
int FixSRDPol::pack_reverse_comm(int n, int first, double *buf)
{
  int i, m, last;
...
void FixSRDPol::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i, j, m;
...
double FixSRDPol::compute_vector(int n)
{
...
void FixSRDPol::velocity_stats(int groupnum)
{
...
double FixSRDPol::newton_raphson(double t1, double t2)
{
...
void FixSRDPol::lineside(double t, double &f, double &df)
{
  double p[2], c[2];
...
void FixSRDPol::triside(double t, double &f, double &df)
{
  double p[2], c[2];
...
double FixSRDPol::distance(int i, int j)
{
...
void FixSRDPol::print_collision(int i, int j, int ibounce, double t_remain, double dt, double *xscoll,
...
#else
void FixSRDPol::print_collision(int, int, int, double, double, double *, double *, double *, int) {}
#endif
